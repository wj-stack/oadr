...
namespace oadr2b
{
  namespace oadr
  {
    ...
    void oadrSamplingRateType::
    oadrMinPeriod (::std::unique_ptr< oadrMinPeriod_type > x)
    {
      this->oadrMinPeriod_.set (std::move (x));
    }
    ...
    void oadrSamplingRateType::
    oadrMaxPeriod (::std::unique_ptr< oadrMaxPeriod_type > x)
    {
      this->oadrMaxPeriod_.set (std::move (x));
    }
  ...
  namespace oadr
  {
      ...
      ::xml_schema_test::flags f)
    {
      for (; p.more_content (); p.next_content (false))
      {
        ...
        // oadrMinPeriod
        //
        if (n.name () == "oadrMinPeriod" && n.namespace_ () == "http://openadr.org/oadr-2.0b/2012/07")
        {
          ::std::unique_ptr< oadrMinPeriod_type > r (
            oadrMinPeriod_traits::create (i, f, this));

          if (!oadrMinPeriod_.present ())
          {
            this->oadrMinPeriod_.set (::std::move (r));
            continue;
          }
        }
        ...
        // oadrMaxPeriod
        //
        if (n.name () == "oadrMaxPeriod" && n.namespace_ () == "http://openadr.org/oadr-2.0b/2012/07")
        {
          ::std::unique_ptr< oadrMaxPeriod_type > r (
            oadrMaxPeriod_traits::create (i, f, this));

          if (!oadrMaxPeriod_.present ())
          {
            this->oadrMaxPeriod_.set (::std::move (r));
            continue;
          }
        }
